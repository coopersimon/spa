/// Coprocessor 15 for the ARM9 processor.
/// Manages cache and TCM.

use arm::armv4::CoprocV4;
...
impl CP15 {
    pub fn new() -> Self {
        Self {
            ...
            data_tcm: MemRegion::default(),
            instr_tcm: MemRegion::default(),
        }
    }
    ...
    fn write_tcm_settings(&mut self, data: u32, info: u32) {
        if info == 1 {
            self.instr_tcm = MemRegion::from_bits_truncate(data);
        } else {
            self.data_tcm = MemRegion::from_bits_truncate(data);
        }
    }
}
