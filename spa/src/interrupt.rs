...
impl MemInterface16 for InterruptControl {
    fn read_halfword(&mut self, addr: u32) -> u16 {
        match addr % 0x10 {
            0 => self.interrupt_enable.bits(),
            2 => self.interrupt_req.bits(),
            8 => if self.interrupt_master {1} else {0},
            10 => 0,
            _ => panic!("interrupt controller: read unreachable address {}", addr)
        }
    }
    fn write_halfword(&mut self, addr: u32, data: u16) {
        match addr % 0x10 {
            0 => self.interrupt_enable = Interrupts::from_bits_truncate(data),
            2 => self.interrupt_req.remove(Interrupts::from_bits_truncate(data)),
            8 => self.interrupt_master = u16::test_bit(data, 0),
            10 => {},
            _ => panic!("interrupt controller: write unreachable address {}", addr)
        }
    }
}
